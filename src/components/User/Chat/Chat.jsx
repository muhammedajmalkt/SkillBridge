    ...
    <div className="h-screen w-full flex bg-gray-200">
      ...
      <div className="w-72 bg-white border-r border-gray-300 flex gap-5 p-6">
        {onlineUser.includes(receiver?._id) && (
          <p className="bg-green-600 h-[10px] w-[10px] mt-9 ml-10 rounded-full absolute"></p>
        )}
        <div className="bg-gray-200 h-fit w-full p-2 flex gap-3 rounded">
          <img
            src={receiver?.image || "default-avatar.png"}
            alt="Receiver Avatar"
            className={`w-10 h-10 rounded-full object-cover bg-gray-500 ${
              onlineUser.includes(receiver?._id) && "border-2 border-green-600 rounded-full"
            }`}
          />
          <div>
            <h2 className="text-xl font-semibold text-gray-800">{receiver?.name}</h2>
            ...
            <div className="flex gap-5">
              {startCall ? (
                <button
                  ...
                  className="bg-red-600 text-white p-2 rounded-lg flex items-center justify-center hover:bg-red-700 transition"
                >
                  End Call
                </button>
              ) : (
               <Link to={`/videocall/${receiver?.name}/${receiver?._id}`}><IoVideocam
                  className="scale-150  cursor-pointer"
                  // onClick={handleVidecall}
                /> </Link> 
              )}
              <BsThreeDotsVertical />
            </div>
          ...
          <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-white">
            {chats?.map((item, index) => {
              ...
              return (
                ...
                >
                  <div
                    className={isSender ? "flex flex-row-reverse gap-2 items-end" : "flex gap-2 items-end"}
                  >
                    <img
                      src={isSender ? user?.image : receiver?.image}
                      className="w-10 h-10 rounded-full object-cover bg-gray-500"
